...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchReports = async () => {
      try {
        const res = await axios.get(`${API_URL}/api/report`); // ✅ Correct endpoint
        setReports(res.data);
      } catch (error) {
    ...
    <div className="p-6 bg-gradient-to-br from-blue-50 via-purple-100 to-pink-50 min-h-screen">
      <div className="flex justify-between mb-6 items-start">
        <div className="text-center w-full">
          ...
          <h4 className="text-md text-gray-600 mt-1">Manage all incoming reports</h4>
        </div>
        <div>
          <button
        ...
        <div className="space-y-6">
          {reports.map((report) => {
            const imageUrl = report.imageUrl?.startsWith("http")
              ? report.imageUrl
              : `${API_URL}${report.imageUrl?.startsWith("/images") ? "" : "/images/"}${report.imageUrl?.trim()}`;
